<!-- custom script here //by dhey soriano -->

<!-- NOTE! SCRIPT NOT FLEXIBLE. TO BE IMPROVED. WRITTEN BY DHEY SORIANO  -->
<script type="text/javascript">
        $("#remedialhelptb").hide();
        function showRemedialHelpTB(){
          var radYes = document.getElementById('remedialhelp_1');
          var radNo = document.getElementById('remedialhelp_2');
          if(radYes.checked){
            $("#remedialhelptb").show();
          }
          if(radNo.checked){
    ...
    @if( isset($field['options']) && $field['options'] = (array)$field['options'] )
        ...
        @foreach ($field['options'] as $value => $label )
            ...
            <label class="radio-inline" for="{{$field['name']}}_{{$optionPointer}}">
                <input  type="radio" onclick="showRemedialHelpTB()" 
            ...
            <div class="radio">
                <label for="{{$field['name']}}_{{$optionPointer}}">
                    <input type="radio" id="{{$field['name']}}_{{$optionPointer}}" onclick="showRemedialHelpTB()" name="{{$field['name']}}" value="{{$value}}" {{$optionValue == $value ? ' checked': ''}}> {!! $label !!}
                </label>
            </div>
